      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">30 equipos registrados</p>
          </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex bg-card card-shadow rounded-lg p-1">
            <button
              ...
              className={`p-2 rounded-md transition-colors ${view === 'cards' ? 'gradient-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setView("table")}
              className={`p-2 rounded-md transition-colors ${view === 'table' ? 'gradient-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
            ...
            {filtered.map((team, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div
                    className="w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold border-2 group-hover:scale-110 transition-transform"
                    style={{ backgroundColor: team.color, borderColor: `${team.color}60`, color: '#fff' }}
                  >
                    {team.number}
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                    team.status === 'active' ? 'bg-success/20 text-success' :
                    team.status === 'disqualified' ? 'bg-destructive/20 text-destructive' :
                    'bg-secondary text-muted-foreground'
                  }`}>
                    {team.status === 'active' ? 'Activo' : team.status === 'disqualified' ? 'Descalificado' : team.status}
                  </span>
                </div>
                <h3 className="font-display text-xl mb-0.5">{team.name}</h3>
                <p className="text-xs text-muted-foreground mb-3">Capitán: {team.captain}</p>
                ...
                <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <UsersIcon className="w-3 h-3" /> {team.membersCount} miembros
                  </div>
                  <div className="font-display text-lg tabular-nums gradient-text">{team.totalPoints.toLocaleString()}</div>
                </div>
              </motion.div>
            ))}
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left px-4 py-3 text-xs text-muted-foreground font-medium">Capitán</th>
                ...
                <tbody>
                  {filtered.map((team) => (
                    <tr key={team.id} className="border-b border-border last:border-0 hover:bg-secondary/30 transition-colors cursor-pointer">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{team.captain}</td>
                      ...
                      <td className="px-4 py-3 text-right font-display text-base tabular-nums">{team.totalPoints.toLocaleString()}</td>
                      <td className="px-4 py-3 text-center">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                          team.status === 'active' ? 'bg-success/20 text-success' : 'bg-destructive/20 text-destructive'
                        }`}>
                          {team.status === 'active' ? 'Activo' : team.status}
                        </span>
                      </td>
                    </tr>
                  ))}
