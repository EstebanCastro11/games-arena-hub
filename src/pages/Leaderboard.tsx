          ...
          {[leaderboard[1], leaderboard[0], leaderboard[2]].map((team, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="w-10 h-10 rounded-lg mx-auto mb-2 flex items-center justify-center text-sm font-bold"
                  style={{ backgroundColor: team.color, color: '#fff' }}
                >
                  {team.number}
                </div>
                <div className="font-display text-lg leading-tight">{team.name}</div>
                <div className="font-display text-xl tabular-nums gradient-text">{team.totalPoints.toLocaleString()}</div>
          ...
          <AnimatePresence>
            {leaderboard.map((team, idx) => {
              ...
              return (
                ...
                >
                  <span className={`font-display text-xl w-8 text-center tabular-nums ${
                    idx < 3 ? "gradient-text" : "text-muted-foreground"
                  }`}>
                    {idx + 1}
                  </span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground truncate">{team.captain} • {team.faculty}</div>
                  </div>
